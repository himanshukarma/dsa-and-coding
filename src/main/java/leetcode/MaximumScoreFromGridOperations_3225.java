...
public class MaximumScoreFromGridOperations_3225 {
	public static void main(String[] args) {
		int[][] grid = { { 0, 0, 0, 0, 0 }, { 0, 0, 3, 0, 0 }, { 0, 1, 0, 0, 0 }, { 5, 0, 0, 3, 0 },
				{ 0, 0, 0, 0, 2 } };
		System.out.println("Result : " + maximumScore(grid));
	}
		...
		for (int c = 1; c < m; c++) {
			...
			for (int curr = 0; curr <= n; curr++) {
				for (int prev = 0; prev <= n; prev++) {
					...
					if (curr <= prev) {
						...
						newdp[curr][prev] = Math.max(newdp[curr][prev], suffMax[prev][0] + gain);
					} else {
						long gain = col[c - 1][curr] - col[c - 1][prev];

						newdp[curr][prev] = Math.max(newdp[curr][prev],
								Math.max(suffMax[prev][curr], prefMax[prev][curr] + gain));
					}
				}
			}
			...
			for (int curr = 0; curr <= n; curr++) {
				...
				for (int prev = 1; prev <= n; prev++) {
					...
					prefMax[curr][prev] = Math.max(prefMax[curr][prev - 1], newdp[curr][prev] - penalty);
				}

				suffMax[curr][n] = newdp[curr][n];

				for (int prev = n - 1; prev >= 0; prev--) {
					suffMax[curr][prev] = Math.max(suffMax[curr][prev + 1], newdp[curr][prev]);
				}
			}
